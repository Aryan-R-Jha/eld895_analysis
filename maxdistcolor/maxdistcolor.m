...
% -------|------------|----------------------------------------------------
% exc    | RGB matrix | Mx3 RGB matrix of colors to exclude. [1,1,1]**
% -------|------------|----------------------------------------------------
...
%        | 'chroma'   | Sorted by chroma,   i.e. calculated from ucs(:,2:3).
...
%
%%% Inputs:
%  N    = ScalarNumeric, the required number of output colors.
...
%  opts = ScalarStructure, with any fields and values as per 'Options' above.
%  OR
...
%
% See also MAXDISTCOLOR_DEMO MAXDISTCOLOR_VIEW
% SRGB_TO_OSAUCS SRGB_TO_CIELAB SRGB_TO_CAM02UCS SRGB_TO_CIEXYZ
...
stpo = struct('start',0, 'Cmin',0, 'Cmax',1, 'Lmin',0, 'Lmax',1,...
	...
	'exc',[1,1,1], 'inc',nan(0,3), 'bitR',6, 'bitG',7, 'bitB',6);
%
...
stpo.cyc = strcmp('closed',stpo.path); % cyc: closed/open -> true/false.
...
cni = fun(inc);
%
tmp = all(bsxfun(@minus,exc,permute(inc,[3,2,1]))<1e-9,2);
assert(~any(tmp(:)),...
...
dgt = 1+fix(log10(nmf));
%
mdcDisplay(stpo,2,'Finished defining the color gamut (%u colors).',nmg)
%
%% Greedy Algorithm %%
%
if nmf==0
...
elseif nmf>0
	fmt = ['The specified RGB gamut contains fewer color nodes than the\n',...
		...
		'The specified RGB gamut contains %u color nodes.'];
	assert(nmt<=nmg,'SC:maxdistcolor:opts:GamutTooSmall',fmt,N,nmt-N,nmg)
	%
	idz = zeros(nmf,1);
	win = zeros(nmf,3);
	chk = zeros(nmf,nmf);
	...
	ucs = [cni;UCS(idz,:)];
	%
	mdcDisplay(stpo,2,'Finished the greedy algorithm (%u iterations).',cnt)
	%
else
...
idp = spl~=1 & idt;
fmt = {' %u day',' %u hour',' %u minute',' %g second';'s','s','s','s'};
...
if any(idf)
	...
	dfn = sprintf(' "%s",',dfc{:});
	error('SC:maxdistcolor:opts:UnknownOptionName',...
...
mdcChkStr('sort', 'none',...
	...
	'farthest','lightness','a','b','j','g','chroma','hue','zip');
%
% Colormap options:
mdcChkRGB('exc');
mdcChkRGB('inc');
...
%
	function idx = mdcField(fld)
		...
		idx = strcmpi(fld,ufc);
		assert(nnz(idx)<2,'SC:maxdistcolor:opts:DuplicateOptionName',...
	...
	function mdcChkRGB(fld)
		...
		if any(idx)
			...
			assert(ndims(map)==2,...
				...
				'The <%s> input must be a numeric matrix ',fld) %#ok<ISMAT>
			if isequal(map,[])
				stpo.(fld) = nan(0,3,stpo.class);
				return
			end
			assert(size(map,2)==3,...
			...
			assert(all(isfinite(map(:))),...
				...
				'The <%s> input must be finite RGB values.',fld)
			assert(isreal(map),...
				'SC:maxdistcolor:map:ComplexValue',...
				'The <%s> input cannot be complex.',fld)
			assert(all(map(:)>=0&map(:)<=1),...
				'SC:maxdistcolor:map:OutOfRange',...
		        'The <%s> input must contain values 0<=RGB<=1',fld)
			stpo.(fld) = cast(map,stpo.class);
		end
	end
	function mdcChkVal(fld,isf,minV,maxV)
		...
		if any(idx)
			...
			assert(isscalar(val),...
				...
				'The <%s> input must be scalar. Numel: %u',fld,numel(val))
			assert(imag(val)==0,...
	...
	function mdcChkStr(fld,varargin)
		% Options check: text value.
		idx = mdcField(fld);
		if any(idx)
			tmp = uvc{idx};
			assert(ischar(tmp)&&ndims(tmp)==2&&size(tmp,1)==1,...
				'SC:maxdistcolor:text:NotText',...
				'The <%s> value must be either a string scalar or char vector.') %#ok<ISMAT>
			if ~any(strcmpi(tmp,varargin))
				error('SC:maxdistcolor:text:NotInList',...
...
clear R G B
mdcDisplay(stpo,2,'Created the entire RGB cube (%u colors).',size(RGB,1))
...
mdcDisplay(stpo,3,'Subsample RGB cube using UCS lightness and chroma...')
tmp = fun([0,0,0;1,1,1]);
...
%
mdcDisplay(stpo,2,'Checked %u permutations (best=%u, metric=%g).',big,trk,low)
%
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%mdcBestPerm
